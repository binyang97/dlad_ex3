...
def rotate_points_along_y(points, angle):
    ...
    return points_rot.numpy() if is_numpy else points_rot, rot_matrix.cpu().numpy()
...
def global2canonical(points, anchors):
    ...
    points, rot_matrix = rotate_points_along_y(points, -ry)
    xyz = np.matmul(xyz[:, :, :3], rot_matrix)
    points[:, :, 0:3] -= xyz
...
def canonical2global(boxes, anchors):
    ...
    boxes, _ = rotate_points_along_y(boxes.unsqueeze(dim=1), ry)
